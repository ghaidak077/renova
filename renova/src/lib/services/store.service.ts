...
export async function createStore(userId: string, data: any) {
  ...
  const { data: profile } = await supabase.from('merchant_profiles').select('id').eq('user_id', userId).single();
...
export async function updateStore(storeId: string, userId: string, data: any) {
    ...
    // Authorization check
    const { data: store, error: fetchError } = await supabase.from('stores').select('user_id').eq('id', storeId).single();
